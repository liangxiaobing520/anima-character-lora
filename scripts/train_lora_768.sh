#!/usr/bin/env bash
# ============================================================
# Anima 单角色 LoRA 训练一条龙 —— **768 分辨率版**
#
# 与 train_lora.sh（640 版）的唯一区别：
#   ① 加 --cache_text_encoder_outputs：caption 先预编码，训练时 Qwen3
#      不常驻显存（省约 1.2 GB），VAE 也会自动移到 CPU（省 0.24 GB）。
#      这是 768 能塞进 8G 卡的关键——640 时权重 5.33G，768 必须降到 3.89G。
#   ② shuffle_caption 必须关：kohya 有硬约束
#      "when caching Text Encoder output, shuffle_caption ... cannot be used"
#   ③ 产物/日志/配置带 -768 后缀，不与 640 版混淆。
#
# 显存账：常驻 3.89G + 768 激活 ~3.5G = 7.4G（8.19G 可用，余量仅 0.6G）
# ============================================================
...
OUT="$GD/output"
CFG="$GD/train/train_${CHAR}_768.toml"
LOGDIR="$GD/logs"
mkdir -p "$OUT" "$LOGDIR"
...
caption_extension = ".txt"
shuffle_caption = false   # ⚠️ 768版：缓存 TE 输出时不能打乱（kohya 硬约束）
keep_tokens = 1
...
echo "输出    : $OUT/${CHAR}_anima_lora-768.safetensors"
echo "=========================================="
...
"$PY" anima_train_network.py \
  ...
  --dataset_config "$CFG" \
  --output_dir "$OUT" \
  --output_name "${CHAR}_anima_lora-768" \
  ...
  --no_metadata \
  2>&1 | tee "$LOGDIR/train_${CHAR}_768.log"
